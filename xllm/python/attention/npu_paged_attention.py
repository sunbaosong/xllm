...
class NpuPagedAttentionBackend(AttentionBackend):
    ...
    def execute_linear(
        ...
        # Work dim-last [1, seq_len, conv_dim] so conv_state keeps the cache's
        # native [N, state_len, conv_dim] layout (no transpose on read/write).
        ...
        for s in range(num_seqs):
            ...
            seg = mixed_qkv[:, t0:t1, :]   # [1, seg_len, conv_dim]
            cs = conv_state[s:s + 1]       # [1, state_len, conv_dim]
            seg_len = t1 - t0
            if is_prefill:
                ...
                if cs_new.shape[1] < conv_state_len:
                    cs_new = F.pad(
                        cs_new,
                        (0, 0, conv_state_len - cs_new.shape[1], 0), value=0,
                    )
                ...
                conv_state[s] = cs[0]
            outs.append(out)
        mixed_qkv = torch.cat(outs, dim=1)   # [1, seq_len, conv_dim]
        hidden_shape = (1, seq_len, -1, head_dim)

        query, key, value = torch.split(
            mixed_qkv, [qkv_dim] * 3, dim=-1
        )

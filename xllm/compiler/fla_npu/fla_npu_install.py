...
Mirrors the structure of ``xllm/compiler/tilelang/tilelang_ascend_install.py``:
a ``ensure_*_ready`` / detect-then-install entry point that the xLLM build
calls for the NPU device. Unlike TileLang, fla_npu is built from source here
(there is no prebuilt-wheel release); the build takes ~3.5 min and is
skipped entirely when the active env already imports the KDA ops.

KDA (conv1d + delta-rule) runs entirely on fla_npu fused ops —
``causal_conv1d`` / ``chunk_kda_fwd`` / ``recurrent_kda`` — with no pure-torch
fallback on the NPU path.
"""
...
PREPARE_FLA_NPU_COMMAND = (
    ...
    f" <fla_npu_src> (FLA_NPU_SOC=<ascend910b|ascend910_93|ascend950>)"
)
...
def _checkout_kda_commit(src: str, env: dict[str, str], cloned_here: bool) -> None:
    ...
    if cloned_here:
        logger.info(f"cloning fla_npu source tree to {src}")
        subprocess.check_call(
            ["git", "clone", "--depth", "1", FLA_NPU_REPO, src], env=env
        )
        # --depth 1 does not reach the KDA commit; unshallow first.
        subprocess.check_call(
            ["git", "fetch", "--unshallow"],
            cwd=src,
            env=env,
    ...
    logger.info(f"checking out fla_npu KDA commit {FLA_NPU_KDA_COMMIT}")
    subprocess.check_call(
        ["git", "fetch", "origin", FLA_NPU_KDA_COMMIT],
        cwd=src,
        env=env,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    subprocess.check_call(
        ["git", "checkout", FLA_NPU_KDA_COMMIT], cwd=src, env=env
    )
...
def prepare_fla_npu(*, force: bool = False) -> Path | None:
    ...
    When the active environment already imports them, this is a no-op (unless
    ``force``). Otherwise the fla_npu source tree is fetched, pinned to the
    commit that landed ``recurrent_kda``, built into a wheel (~3.5 min, no
    incremental benefit — the kernel rebuilds every time), and
    force-installed.
    ...
    function only builds and installs the wheel. Returns None on success.
    """
    set_npu_envs()
